...
# HEIGHT = 333
# WIDTH = 3250
# data_dir = "TrackNetV2_Dataset_new"
...
def list_dirs(directory):
    ...
    return sorted(
        [
            os.path.normpath(os.path.join(directory, path)).replace("\\", "/")
...
def frame_first_RGB(input, input_type, HEIGHT, WIDTH):
    ...
    for n in range(input.shape[0]):
        ...
        for f in range(0, input.shape[-1], 3):
            img = input[n, :, :, f : f + 3]
            tmp_frame = np.concatenate(
                (tmp_frame, img.reshape(1, HEIGHT, WIDTH, 3)), axis=0
            )
        tmp_img = np.concatenate(
            (tmp_img, tmp_frame.reshape(1, num_frame, HEIGHT, WIDTH, 3)), axis=0
        )

    return tmp_img


def frame_first_RGBD(input, input_type, HEIGHT, WIDTH):
    """Helper function for transforming x to cv image format.
...
def frame_first_Gray(input, input_type, HEIGHT, WIDTH):
...
def generate_frames(video_file):
    ...
    while success and num_frames != len(label_df):
        ...
        if success:
            cv2.imwrite(
                f"{frame_dir}/{num_frames}.png", image, [cv2.IMWRITE_PNG_COMPRESSION, 0]
            )
            num_frames += 1
...
def FocalWBCE(y, y_pred):
    ...
    loss = (-1) * (
        ...
        * (torch.clamp(1 - y_pred, 1e-7, 1) ** gamma)
        * y
        * torch.log(torch.clamp(y_pred, 1e-7, 1))
        + torch.square(y_pred)
        * ((torch.clamp(y_pred, 1e-7, 1)) ** gamma)
        * (1 - y)
...
def train(
    epoch, model, optimizer, loss_fn, data_loader, input_type, display_step, save_dir
):
    model.train()
...
def evaluation(model, data_loader, tolerance, input_type):
    ...
    for step, (i, x, y, c) in enumerate(data_prob):
        ...
        y_pred = y_pred > 0.3
        # y_pred = y_pred > 0.5
        tp, tn, fp1, fp2, fn = get_confusion_matrix(
            i, y_pred, y, c, tolerance, input_type=input_type
        )
        TP.extend(tp)
        TN.extend(tn)
        FP1.extend(fp1)
        ...
        data_prob.set_postfix(
            TP=len(TP), TN=len(TN), FP1=len(FP1), FP2=len(FP2), FN=len(FN)
        )

    accuracy, precision, recall = get_metric(
        len(TP), len(TN), len(FP1), len(FP2), len(FN)
    )
    print(
...
def get_frame_unit(frame_list, num_frame, HEIGHT, WIDTH):
    """Sample frames from the video.
...
#     """ Get coordinates of the maximum value in the heatmap.

#         args:
#             heatmap - A numpy.ndarray of a single heatmap with shape (H, W)

#         returns:
#             ints specifying the coordinates of the maximum value in the heatmap
#     """
#     # Check if the heatmap has any non-zero values
#     if np.amax(heatmap) == 0:
#         # No response in the heatmap
#         return 0, 0
#     else:
#         # Get the index of the maximum value in the heatmap
#         max_idx = np.argmax(heatmap)

#         # Convert the index into 2D coordinates (row, col)
#         max_y, max_x = np.unravel_index(max_idx, heatmap.shape)

#         return int(max_x), int(max_y)


# 元々のget_object_center↓
def get_object_center(heatmap):
    """Get coordinates of the maximum value in the heatmap.
    ...
    Returns:
        Tuple of ints specifying the coordinates (x,y) of the maximum value
    """
    if np.amax(heatmap) == 0:
        return 0, 0
    else:
...
def show_prediction(x, y, y_pred, y_coor, input_type, save_dir):
    """Visualize the input sequence with its predicted heatmap.
    Save as a gif image.
    ...
    args:
        ...
        y_coor - A tf.EagerTensor of ground-truth coordinate sequences
        input_type - A str specifying input type
            '2d' for stacking all the frames at RGB channel dimension result in shape (H, W, F*3)
            '3d' for stacking all the frames at extra dimension result in shape (F, H, W, 3)
        save_dir - A str specifying the save directory
    """
    imgs = []
    x, y, y_pred, y_coor = (
    ...
    for f in range(y_coor.shape[0]):
        # print(f'Frame {f}: Input coordinates: (x={y_coor[f][0]}, y={y_coor[f][1]})')
        ...
        # Mark ground-truth label on the top-left image (original input)
        if int(y_coor[f][0]) > 0 and int(y_coor[f][1]) > 0:
            # Add circle for ground-truth on the top-left original input image
            # print(f'Frame {f}: Drawing circle at: (x={int(y_coor[f][0])}, y={int(y_coor[f][1])})')
            cv2.circle(
                tmp_x, (int(y_coor[f][0]), int(y_coor[f][1])), 5, (255, 0, 0), -1
            )  # Red circle for ground-truth

        # Mark ground-truth label in the concatenated bottom-left predicted heatmap (already present)
