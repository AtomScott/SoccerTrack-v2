...
def plot_frame_detections(
    ...
    for _, det in detections.iterrows():
        ...
        # Use team colors for class_name and track colors for track_id
        team_color = tuple(team_colors.get(det["class_name"], [128, 128, 128]))  # Gray as fallback
        track_color = tuple(track_colors[str(det["id"])])  # Will generate new color if needed

        # You can choose which color to use for what purpose, for example:
        bbox_color = team_color
        label_color = track_color  # or vice versa

        # Draw bounding box
        cv2.rectangle(frame_with_boxes, (x1, y1), (x2, y2), bbox_color, line_thickness)

        # Add label with confidence score
        # label = f"{det['class_name']} {det['conf']:.2f}"
        label = f"{det['id']}"
        text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, line_thickness)[0]
        cv2.rectangle(frame_with_boxes, (x1, y1 - text_size[1] - 4), (x1 + text_size[0], y1), label_color, -1)
        cv2.putText(
            frame_with_boxes,
            label,
            (x1, y1 - 2),
...
def plot_bboxes_on_video(
    ...
    track_colors = defaultdict(lambda: [np.random.randint(0, 255) for _ in range(3)], track_colors or {})
    ...
    # Load detections with optional event_period filtering
    ...
    if first_frame_only:
        try:
            ...
            if frame is not None:
                ...
                annotated_frame = plot_frame_detections(frame, frame_dets, team_colors, track_colors)
