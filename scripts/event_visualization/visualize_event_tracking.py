...
def display_label(frame, label, label_type):
    """
    ...
    """
    if label_type == 'label':
        position = (80, 600)  # ラベルの表示位置
        color = (128, 0, 0)
    elif label_type == 'detection':
        position = (750, 600)  # 検出ラベルの表示位置（ラベルと重ならない位置）
        color = (0, 0, 128)
    else:
        return
    cv2.putText(frame, label, position, cv2.FONT_HERSHEY_SIMPLEX, 3, color, 2)
...
def visualize_event_tracking(tracking_df: pd.DataFrame, labels_df: pd.DataFrame, detections_df: pd.DataFrame, output_path: str) -> None:
    ...
    for frame_num in range(fps * video_duration_seconds):
        ...
        if label_index < len(labels):
            ...
            if frame_num == label_position / 40:
                display_label(frame, label['label'], 'label')
                label_index += 1
        ...
        if detection_index < len(detections):
            ...
            if frame_num == detection_position / 40:
                display_label(frame, detection['label'], 'detection')
                detection_index += 1
