...
def evaluate(
    ...
    predictions_half_1, predictions_half_2 = predictions2vector(
        outputs, num_classes=num_classes, version=version, EVENT_DICTIONARY=EVENT_DICTIONARY,
        framerate=framerate)
    ...
    results = {
        ...
        "a_mAP_per_class_unshown": {event: value for event, value in zip(EVENT_DICTIONARY.keys(), a_mAP_per_class_unshown)} if version == 2 else None,
    }
    return results
...
def predictions2vector(predictions, num_classes=17, version=2, framerate=2, EVENT_DICTIONARY={}):
    ...
    for prediction in predictions["predictions"]:
        ...
        time = int(float(prediction["position"]))
        event = prediction["label"]

        half = int(prediction["gameTime"][0])
...
def compute_precision_recall_curve(targets, closests, detections, delta):
    ...
    for c in np.arange(num_classes):
        ...
        for target, closest, detection in zip(targets, closests, detections):
            tmp_detections, tmp_n_gt_labels_visible, tmp_n_gt_labels_unshown = compute_class_scores(target[:, c],
                                                                                                    closest[:, c],
                                                                                                    detection[:, c],
                                                                                                    delta)
...
def average_mAP(targets, detections, closests, framerate=2, deltas=np.arange(5) * 1 + 1):
    ...
    for c in np.arange(len(mAP_per_class[0])):
        ...
        a_mAP_per_class.append(float(integral_per_class / ((len(mAP_per_class) - 1))))
    ...
    for c in np.arange(len(mAP_per_class_visible[0])):
        ...
        a_mAP_per_class_visible.append(float(integral_per_class_visible / ((len(mAP_per_class_visible) - 1))))
    ...
    for c in np.arange(len(mAP_per_class_unshown[0])):
        ...
        a_mAP_per_class_unshown.append(float(integral_per_class_unshown / ((len(mAP_per_class_unshown) - 1))))
