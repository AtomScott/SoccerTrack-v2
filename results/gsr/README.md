...
- `rescore_v1_2/`: the saved whole-half predictions of M10 (132877, both halves) rescored with `score_one.py` against release v1.2, which re-synchronised M10 with its video (no frame shift, ids as saved). `compile_full_table.py` takes these values in place of the cloud runs' own scores and keeps the source tag. The M10 files in `prefix_rescoring/`, `score_30s_132877_{1st,2nd}.json`, `score_30s_128057_2nd.json` and their rows in `sweep30s_all_matches.csv` are v1.2 rescorings as well; each of those JSONs names its `gt`, `pred` and a `note`. The other halves score identically against v1.2 (spot-checked 2026-09-24).
